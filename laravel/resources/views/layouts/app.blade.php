...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        main { width: min(1400px, 100%); margin: 0 auto; padding: 1.25rem; }
        .dashboard-grid { display: grid; grid-template-columns: minmax(0, 2fr) minmax(340px, 1fr); gap: 1.25rem; align-items: start; }
        ...
        .instruction.pending { border-left: 4px solid #dc2626; padding-left: .8rem; }
        ...
        @media (max-width: 900px) { .dashboard-grid { grid-template-columns: 1fr; } nav form { margin-left: 0; } }
    </style>
</head>
<body>
<header>
    <nav>
        ...
        @auth
            @if(auth()->user()->isManagement())
                <a href="{{ route('instructions.create') }}">Management Instruction</a>
            @endif
        ...
        @else
            <a href="{{ route('login') }}">Management Login</a>
        @endauth
    </nav>
</header>
<main>
